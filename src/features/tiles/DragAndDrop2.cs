...
public partial class DragAndDrop2 : Node
{
    ...
    public Variant GetDragData(Vector2 atPosition){
        ...
        _pieceRoot.SetDragPreview(/* dragPreview */previewNode); //well none of this works...
        GD.Print((_pieceRoot as Tile).Type.ToString());
		return _pieceRoot;        
    }

	public bool CanDropData(Vector2 atPosition, Variant data){ //this can drop data on it's self since the root tile is a separate control node with offset boundaries
	...
	public void DrawDebugGizmo(){
        var x = _pieceRoot.Position.X;
        var y = _pieceRoot.Position.Y;

        _pieceRoot.DrawPolygon(_hexPoints, [new Color(1, 0.5f, 0.5f, 0.4f)]);		
	}
}
